#!/usr/bin/env python3
"""
Extract all successful rotation task outputs with 12-frame EPS visualizations
"""
...
QUESTIONS_DIR = Path("/Users/access/VMEvalKit/data/questions/rotation_task")
TEMP_DIR = Path("/tmp/rotation_success_outputs")

# Success threshold (GPT4O scores are typically 1-5, with 5 being best)
SUCCESS_THRESHOLD = 4

def find_successful_rotation_tasks():
    """Find all successful rotation tasks across all models"""
    successful_tasks = []
    ...
    for model_dir in model_dirs:
        ...
        rotation_dir = model_dir / "rotation_task"
        
        if not rotation_dir.exists():
            continue
        
        # Check each rotation task
        for task_dir in sorted(rotation_dir.iterdir()):
...
def create_12frame_visualization(video_path, output_eps, output_png):
    ...
    fig.suptitle('12-Frame Video Sequence - Rotation Task', fontsize=16)
...
def organize_successful_outputs(successful_tasks):
    ...
    for task_info in successful_tasks:
        ...
        output_model_dir = OUTPUT_DIR / model / "rotation_task" / task_id
    ...
    # Create README
    readme_content = f"""# Successful Rotation Task Outputs

Total successful rotation tasks: {len(successful_tasks)}
...
1. **original_question/** - The original 3D rotation puzzle setup
...
def main():
    print("Scanning for successful rotation tasks...")
    print("=" * 60)
    
    successful_tasks = find_successful_rotation_tasks()
    
    print("\n" + "=" * 60)
    print(f"Found {len(successful_tasks)} successful rotation tasks")
    print("=" * 60)
