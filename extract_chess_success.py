#!/usr/bin/env python3
"""
Extract all successful chess task outputs with 12-frame EPS visualizations
"""
...
QUESTIONS_DIR = Path("/Users/access/VMEvalKit/data/questions/chess_task")
TEMP_DIR = Path("/tmp/chess_success_outputs")

# Success threshold (GPT4O scores are typically 1-5, with 5 being best)
SUCCESS_THRESHOLD = 4

def find_successful_chess_tasks():
    """Find all successful chess tasks across all models"""
    successful_tasks = []
    ...
    for model_dir in model_dirs:
        ...
        chess_dir = model_dir / "chess_task"
        
        if not chess_dir.exists():
            continue
        
        # Check each chess task
        for task_dir in sorted(chess_dir.iterdir()):
...
def extract_frames_from_video(video_path, output_dir, num_frames=12):
...
def create_12frame_visualization(video_path, output_eps, output_png):
    ...
    fig.suptitle('12-Frame Video Sequence', fontsize=16)
...
def organize_successful_outputs(successful_tasks):
    ...
    for task_info in successful_tasks:
        ...
        output_model_dir = OUTPUT_DIR / model / "chess_task" / task_id
    ...
    # Create README
    readme_content = f"""# Successful Chess Task Outputs

Total successful chess tasks: {len(successful_tasks)}
...
1. **original_question/** - The original chess puzzle setup
...
def main():
    print("Scanning for successful chess tasks...")
    print("=" * 60)
    
    successful_tasks = find_successful_chess_tasks()
    
    print("\n" + "=" * 60)
    print(f"Found {len(successful_tasks)} successful chess tasks")
    print("=" * 60)
