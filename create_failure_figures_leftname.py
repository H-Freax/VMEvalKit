#!/usr/bin/env python3
"""
Create CLEAN failure case visualizations - MODEL NAME ON LEFT
"""
...
def create_failure_figure(task_name, failure_cases, output_path):
    """Create CLEAN figure - model name on the very left"""
    ...
    # Create figure
    fig = plt.figure(figsize=(30, 3.5 * num_models))
    ...
    # Create main grid: 16 columns
    # Column 0: Empty space for model name (will use text on left margin)
    # Columns 1-2: Initial/Expected
    # Column 3: GAP
    # Columns 4-15: 12 video frames
    gs = gridspec.GridSpec(num_models, 15,
                          hspace=0.3, wspace=0.2,
                          left=0.12, right=0.98,  # More left margin for model names
    ...
    for model_idx, (model_name, failure_data) in enumerate(models_with_failures):
        ...
        # Model name on the very left - vertically centered with the row
        model_display = model_name.replace('-', '\n').replace('_', '\n').upper()
        y_pos = (num_models - model_idx - 0.5) / num_models * 0.92 + 0.04
        
        fig.text(0.01, y_pos, model_display,
...
def main():
    ...
    print("\n" + "=" * 60)
    print("Creating failure figures with left-aligned names...")
    print("=" * 60)
