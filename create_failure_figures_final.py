#!/usr/bin/env python3
"""
Create CLEAN failure case visualizations - FINAL VERSION
- Model names above images (no overlap)
...
def create_failure_figure(task_name, failure_cases, output_path):
    """Create CLEAN figure - all images same size, no overlap"""
    ...
    # Create figure: wider and taller for equal-sized images
    fig = plt.figure(figsize=(28, 3.5 * num_models))
    ...
    # Create main grid: rows for each model, 14 equal columns
    # All 14 columns are same width for uniform image sizes
    ...
    for model_idx, (model_name, failure_data) in enumerate(models_with_failures):
        ...
        # Model name - positioned ABOVE the row (no overlap)
        model_display = model_name.replace('-', ' ').replace('_', ' ').upper()
        y_pos = 0.92 - (model_idx / num_models) * 0.92 + 0.025
        ...
        # Columns 2-13: 12 video frames (all same size)
        if video_frames:
...
def main():
    ...
    print("\n" + "=" * 60)
    print("Creating FINAL clean failure figures...")
    print("=" * 60)
