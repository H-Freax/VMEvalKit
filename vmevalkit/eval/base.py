...
class BaseEvaluator(ABC):
    ...
    def evaluate_selective(self, models: Optional[List[str]] = None,
                          task_types: Optional[List[str]] = None,
                          task_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        """Evaluate specific models/tasks based on filters.
        ...
        if models is None:
            models = [d.name for d in self.experiment_dir.iterdir() if d.is_dir()]
        ...
        for model_name in models:
            ...
            try:
                # Get model results with filters
                results = self._evaluate_model_selective(
                    model_name, task_types, task_ids
                )
                all_results[model_name] = results
            except Exception as e:
                logger.error(f"Failed to evaluate {model_name}: {e}")
        ...
        output_path = self.output_dir / self.experiment_name / f"{self.evaluation_metadata['evaluator_type']}_selective.json"
        output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _evaluate_model_selective(self, model_name: str,
                                 task_types: Optional[List[str]] = None,
                                 task_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        """Evaluate a model with task type and ID filters."""
        model_dir = self.experiment_dir / model_name
        if not model_dir.exists():
            raise ValueError(f"Model directory not found: {model_dir}")
        ...
        # Iterate through task types (with filtering)
        for task_type_dir in model_dir.iterdir():
            if not task_type_dir.is_dir():
                continue
            ...
            # Skip if task type not in filter
            if task_types and task_type not in task_types:
                continue
                
            results["evaluations"][task_type] = {}
            
            # Iterate through tasks (with filtering)
            for task_dir in task_type_dir.iterdir():
                if not task_dir.is_dir():
                    continue
                ...
                # Skip if task ID not in filter
                if task_ids and task_id not in task_ids:
                    continue
                
                # Check if already evaluated
                eval_path = self.output_dir / self.experiment_name / model_name / task_type / task_id / f"{self.evaluation_metadata['evaluator_type']}.json"
                if eval_path.exists():
                    logger.info(f"Skipping already evaluated task: {model_name}/{task_type}/{task_id}")
                    continue
                ...
                try:
                    ...
                    self._save_individual_result(
                        model_name, task_type, task_id, eval_result
                    )
    ...
    def _save_individual_result(self, model_name: str, task_type: str, task_id: str, result: Dict[str, Any]):
        """Save a single evaluation result immediately."""
